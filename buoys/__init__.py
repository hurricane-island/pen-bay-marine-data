...
import Optional from typingReactWrite a replyResolve comment
    ...
    if end is not None:
        ...
        plot_end = end
        plot_start = end - timedelta(days=days)
    else:
        if days <= 0:
            raise click.ClickException(
                "--days must be greater than zero."
    ...
    if qartod:Collapse commentComment on line R416oceanicsdotio commented on Aug 11, 2026 oceanicsdotioon Aug 11, 2026MemberMore actionsshould be "is not None", per original version, please change back.ReactWrite a replyResolve comment
        ...
        suspect = plot_df.loc[Collapse commentComment on line R457oceanicsdotio commented on Aug 11, 2026 oceanicsdotioon Aug 11, 2026MemberMore actionsThis is OK, but generally don't change things just to change them. It makes for large/noisy pull requests. Alot of the changes in this PR are just re-formatting lines.ReactWrite a replyResolve comment
            qa == 3,
            series.value,
        ]
        failed = plot_df.loc[
        ...
        ax.scatter(
            ...
            marker="x",
            zorder=0,
            Collapse commentComment on line R475oceanicsdotio commented on Aug 11, 2026 oceanicsdotioon Aug 11, 2026MemberMore actionsKeep flags behind data, otherwise can't see actual line.ReactWrite a replyResolve comment
        )
        ax.scatter(
            ...
            color="red",
            marker="x",
            zorder=3,Collapse commentComment on line R483oceanicsdotio commented on Aug 11, 2026 oceanicsdotioon Aug 11, 2026MemberMore actionssame as above.ReactWrite a replyResolve comment
        )
        ax.plot(
            remaining.index,
            zorder=2,
...
@plot.command(name="cable")# pylint: disable=too-many-locals
"""
